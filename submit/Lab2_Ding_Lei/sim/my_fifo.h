...
template<class T>
my_fifo_t<T>::my_fifo_t(int size) {
    max_size = size;
    num = 0;
    els = new T[size];
    head = tail = 0;
...
// same as constructor
template<class T>
my_fifo_t<T>::my_fifo_t(char *_name, int size) {
    max_size = size;
    num = 0;
    els = new T[size];
    head = tail = 0;
...
// same as constructor
template<class T>
void my_fifo_t<T>::init(int size) {
    max_size = size;
    num = 0;
    els = new T[size];
    head = tail = 0;
...
// same as constructor
template<class T>
void my_fifo_t<T>::init(int size, T empty) {
    max_size = size;
    num = 0;
    els = new T[size];
    head = tail = 0;
...
// same as constructor
template<class T>
void my_fifo_t<T>::init(char *_name, int size, T empty) {
    init(size, empty);
    name = _name;
}
...
// same as constructor
template<class T>
void my_fifo_t<T>::init(char *_name, int size) {
    init(size);
    name = _name;
}
...
// same as constructor
template<class T>
void my_fifo_t<T>::init_time(char *_name, int size, T empty) {
    name = _name;
    max_size = size;
    num = 0;
    els = new T[size];
...
template<class T>
void my_fifo_t<T>::print() {
    cout << ":my_fifo_t: num = " << size() << ", max_size = " << max_size << "\n";
}

template<class T>
inline void my_fifo_t<T>::push_back(T value) {
...
template<class T>
inline void my_fifo_t<T>::push_front(T value) {
...
template<class T>
inline void my_fifo_t<T>::tpush_front(int time, T value) {
    static char *_func_name = "my_fifo_t::push";
...
T my_fifo_t<T>::front() {
    ...
    if (num == 0) ERROR_ARGS(("front of empty fifo (%s)", name));
    return (els[head]);
}

template<class T>
T my_fifo_t<T>::dequeue() {
...
// time dequeue
template<class T>
T my_fifo_t<T>::tdequeue(int cur_time) {
    //  static char *_func_name = "my_fifo_t::dequeue";
...
// *time dequeue
template<class T>
T my_fifo_t<T>::tdequeue(int *time) {
    static char *_func_name = "my_fifo_t::dequeue";
    T r = els[head];
    *time = time[head];
    num--;
...
template<class T>
void my_fifo_t<T>::enqueue(T value) {
    push_back(value);
}

template<class T>
void my_fifo_t<T>::tenqueue(int t, T value) {
    times[tail] = t;
    push_back(value);
}

template<class T>
inline bool my_fifo_t<T>::space_p() {
    return (num < max_size);
}

template<class T>
inline bool my_fifo_t<T>::full_p() {
    return !(space_p());
}

template<class T>
inline bool my_fifo_t<T>::empty() {
    return (num == 0);
}

template<class T>
inline bool my_fifo_t<T>::tempty(int cur_time) {
    return ((num == 0) || (times[head] > cur_time));
}

template<class T>
inline int my_fifo_t<T>::size() {
    return (num);
}

template<class T>
inline int my_fifo_t<T>::space() {
    return (max_size - num);
}

template<class T>
inline int my_fifo_t<T>::get_head() {
    wraparound_p = false;
    return (head);
}

template<class T>
inline int my_fifo_t<T>::get_tail() {
    if (num == max_size)
        return (-1);
    else
        return (tail);
...
template<class T>
inline int my_fifo_t<T>::next(int cur) {
    if (cur == tail) {
        if (wraparound_p)
            return (-1);
        else {
...
template<class T>
inline T my_fifo_t<T>::sel(int cur) { // no error checking
    return (els[cur]);
}

template<class T>
inline void my_fifo_t<T>::erase(int cur) {
...
template<class T>
inline T my_fifo_t<T>::sel_offset(int i) { // no error checking
    int cur = (head + i) % max_size;
    return (els[cur]);
}

template<class T>
inline void my_fifo_t<T>::erase_offset(int i) {
